...
class LinkedIn
{
     ...
     */
    public function share(array $data, $author = "")
    {
        if($author == ""){
    ...
    public function upload($file, $author = "")
    {
        ...
        if($author == ""){
            ...
            $data['registerUploadRequest']['owner'] = $author;
        }
    ...
    {
        $result = $this->get('v2/organizationalEntityAcls?q=roleAssignee&role=ADMINISTRATOR&projection=(elements*(organizationalTarget~(id,localizedName,vanityName,logoV2(original~:playableStreams,cropped~:playableStreams,cropInfo))))');
        return $result;
    }
}
