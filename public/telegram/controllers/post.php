...
class post extends MX_Controller {
	...
	public function __construct(){ 
		...
		$this->module_icon = "fab fa-telegram";
	...
	public function index($ids = ""){
		...
		$data = array(
			...
			'accounts'     => $this->model->fetch("id, pid, type, username, avatar, ids", $this->tb_accounts, "uid = '".session("uid")."'")
				...
				foreach ($accounts as $account_id) {
					...
					if(!empty($telegram_account)){
						...
						if(is_array($result)){
							...
							$data['result'] = json_encode(array("message" => "successfully", "id" => $result->message_id, "url" => "https://web.telegram.org/#/im?p=@".$result->chat->username));
	...
	public function cron(){
		...
		if(!empty($schedule_list)){
			...
			foreach ($schedule_list as $key => $schedule) {
				...
				$telegram = new Telegram_API($schedule->access_token);

				$result = $telegram->post($schedule, $schedule->pid);
				...
				if(is_array($result) && $result["status"] == "error"){
					...
					update_setting("telegram_post_error_count", get_setting("telegram_post_error_count", 0, $schedule->uid) + 1, $schedule->uid);

					update_setting("telegram_post_count", get_setting("telegram_post_count", 0, $schedule->uid) + 1, $schedule->uid);
					...
					update_setting("telegram_post_success_count", get_setting("telegram_post_success_count", 0, $schedule->uid) + 1, $schedule->uid);

					update_setting("telegram_post_count", get_setting("telegram_post_count", 0, $schedule->uid) + 1, $schedule->uid);

					update_setting("telegram_post_{$type}_count", get_setting("telegram_post_{$type}_count", 0, $schedule->uid) + 1, $schedule->uid);
