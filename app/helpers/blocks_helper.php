				...
				if(file_exists($folder."/config.json")){
					...
					if(isset($data->menu) && !empty($data->menu)){
						...
						if(permission(preg_replace("/\s+/","_",strtolower(trim(ucfirst($data->menu->title))))."_enable")){
							$item  = "";
							$item .= "<li class='".((segment(1) == strtolower($folder_name))?"active":"")."'>";
							if(isset($data->menu->icon)){
							$menu_title = preg_replace("/\s+/","_",strtolower(trim(ucfirst($data->menu->title))));
							$item .= "<a href='javascript:void(0);' class='menuPopover'>
								<i class='".$data->menu->icon."' " . (get_option('sidebar_icons_color', 0)==1 && isset($data->menu->color)?"style='color: ".$data->menu->color."'":"") . " aria-hidden='true'></i>
								<span class='name'>".lang($menu_title)."</span>
								<span class='menu-arrow'></span></a>";
					        }

					        $item .= "<ul class='nav-second-level ".((segment(1) == strtolower($folder_name))?"collapse in":"collapse")."' aria-expanded='false'>";
					        ...
					        if(isset($data->submenu) && !empty($data->submenu)){
					        	...
					        	foreach ($data->submenu as $link => $title) {
					        		...
					        		if(permission($link)){
										...
										$item .= "<li class=".((segment(1) == strtolower($folder_name) && segment(2) == end($link_array))?"active":"").">
											<a href='".PATH.$link."'><i class='fa fa-angle-right' aria-hidden='true'></i> <span>".$text."</span></a></li>";
					            	}
					            	//End permission
					            }
					    	}
